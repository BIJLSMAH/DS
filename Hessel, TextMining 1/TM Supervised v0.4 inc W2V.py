...
df['KOD'] = df['Korte omschrijving Details'].str.lower()
df['KOD'] = df['KOD'].apply(lambda x: re.sub('[^a-z]+',' ',x))
df['KOD'] = df['KOD'].str.split()
...
inctrainDF['text'] = df['KOD']

#%% Word2Vec
# warnings.fiterwarnings(action='ignore')
# Create CBOW model
# Create Skip gram model

# De stap hieronder bouwt de vocabulary op.
ldoc = list()
for d in inctrainDF['text']:
    lwords = list()
    words = d.split()
    for w in words:
        lwords.append(w)
...
model = gensim.models.Word2Vec(ldoc, min_count=2, size=150, window=10, workers=10, iter=10)
model.train(ldoc, total_examples=model.corpus_count, epochs=200)
model.save("w2vmodel")
...
# een dictionary word naar 150 dimensional vector. Deze kunnen we gebruiken
# voor 
...
print("2.0. FEATURE ENGINEERING: Generate W2V Vectors . . .")
w2v_vect = MeanEmbeddingVectorizer(w2v)
w2v_vect.fit(train_x, train_y)

# transform the training and validation data using count vectorizer object
xtrain_w2v =  w2v_vect.transform(train_x)
xvalid_w2f =  w2v_vect.transform(valid_x)
