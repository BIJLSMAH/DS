...
df = pd.read_csv(r'data\MAAND OPEN PRD 2014-2019.csv', header=0, index_col=0, parse_dates=True, squeeze=True)
#df['maand'] = pd.to_datetime(df['maand']
df.index.min(), df.index.max()

# Indexing met tijdreeksgegevens
# index staat al goed op 'maand' 
...
mod = sm.tsa.statespace.SARIMAX(y,order=(6,0,6), seasonal_order=(6,0,6,12),enforce_stationarity = False,enforce_invertibility= False)
# mod = sm.tsa.statespace.SARIMAX(y,order=(1,0,5), seasonal_order=(0,0,5,12),enforce_stationarity = False,enforce_invertibility= False)
results=mod.fit()
...
ax.set_xlabel('maand')
ax.set_ylabel('openstaande incidenten')
plt.legend()
plt.show()
